...
class APICallLogTest(TestCase):
    ...
    def test_missing_api_key(self):
        with self.assertRaises(ValidationError):
            log = APICallLog(
                endpoint="/api/v1/data", national_id="1234567890", success=False
            )
            log.full_clean()
    ...
    def test_missing_endpoint(self):
        with self.assertRaises(ValidationError):
            log = APICallLog(
                api_key="test_api_key", national_id="1234567890", success=False
            )
            log.full_clean()
    ...
    def test_duplicate_national_id(self):
        ...
        self.assertEqual(APICallLog.objects.filter(national_id="1234567890").count(), 2)
    ...
    def test_str_representation_success(self):
        ...
        expected_str = (
            ...
            f"(Success): {self.log.national_id}"
        )
    ...
    def test_delete_log(self):
        self.log.delete()
        self.assertEqual(APICallLog.objects.filter(api_key="test_api_key").count(), 0)
