...
from .utils import governorates, is_valid_national_id, log_api_call
...
class NationalIDView(APIView):
    """
    Endpoint to validate an Egyptian
    National ID number and extract information if valid.

    To retrieve your API Key, run the endpoint GET '/' to obtain the key.
    Once you have the key, use it in the request header as follows:
    ...
    @extend_schema(
        examples=[
            OpenApiExample(
                "Example National ID", value={"national_id": "29001011234567"}
            )
        ],
    ...
    def post(self, request, *args, **kwargs):
        api_key = (
            ...
            .replace(API_KEY_HEADER, "")
            .strip()
        )
        endpoint = request.path
        if not api_key:
            return Response(
                {"error": "API key is missing."}, status=status.HTTP_400_BAD_REQUEST
            )
        try:
